...
def get_possible_combinations(get_combinations_cmd, robot_names_7dof=[]):
    ...
    for robot_description_package in robot_description_packages:
        ...
        for macro_file in robot_description_macro_files:
            character_after_package_prefix = macro_file[len(package_prefix)]
            if macro_file.startswith(package_prefix) and not character_after_package_prefix.isnumeric():
                robot_name = 'kuka_' + macro_file.split('_macro')[0]
                if robot_name in robot_names:
                    arg_combination = KukaArgCombination(robot_description_package=robot_description_package,
                                                         robot_description_macro_file=macro_file,
                                                         robot_name=robot_name,
                                                         controllers_file=controller_file_7dof if robot_name in robot_names_7dof else controller_file_6dof)
...
launch_file_name = "test_ros2_control_kuka.launch.py"
get_combinations_cmd = ['ros2', 'launch', package_name, launch_file_name, '--show-args']
possible_combinations = get_possible_combinations(get_combinations_cmd, robot_names_7dof=robot_names_7dof)
...
def generate_test_description(launch_testing_params):
    ...
    return LaunchDescription(
        declared_arguments + [ReadyToTest(), robot_driver]
    )
